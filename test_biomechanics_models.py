"""
Comprehensive test suite for biomechanics models.
Tests accuracy of:
...
class TestAngleCalculation:
    ...
    def test_elbow_flexion_angles(self):
        ...
        test_cases = [
            ...
            ((0, 0), (1, 0), (0.5, 0.87), 120, "120-degree flex"), # Partial flex
        ]
...
class TestRepetitionCounting:
    """Test repetition counting accuracy"""
    ...
    def test_basic_rep_counting(self):
        ...
        if counter.reps == expected_reps:
            self.passed += 1
            print(f"✅ Basic Rep Count: {counter.reps} reps (expected {expected_reps})")
        else:
            self.failed += 1
            self.errors.append(f"Basic rep count failed: got {counter.reps}, expected {expected_reps}")
            print(f"❌ Basic Rep Count: {counter.reps} reps (expected {expected_reps})")
    
    def test_multiple_reps(self):
        """Test counting multiple repetitions"""
        counter = RepCounter(min_angle=70, max_angle=140, movement_threshold=2)
        ...
        if counter.reps == expected_reps:
            self.passed += 1
            print(f"✅ Multiple Reps Count: {counter.reps} reps (expected {expected_reps})")
        else:
            self.failed += 1
            self.errors.append(f"Multiple reps count failed: got {counter.reps}, expected {expected_reps}")
            print(f"❌ Multiple Reps Count: {counter.reps} reps (expected {expected_reps})")
    ...
    def test_posture_ignore_incorrect(self):
        ...
        posture_ok = [
            ...
            True, True, True, False, False,  # Bad posture during rep 2
        ]
        ...
        if counter.reps == expected_reps:
            self.passed += 1
            print(f"✅ Posture Check: {counter.reps} reps counted (expected {expected_reps})")
        else:
            self.failed += 1
            self.errors.append(f"Posture check failed: got {counter.reps}, expected {expected_reps}")
            print(f"❌ Posture Check: {counter.reps} reps counted (expected {expected_reps})")
    
    def test_noise_immunity(self):
        """Test immunity to small angle jitter"""
        counter = RepCounter(min_angle=70, max_angle=140, movement_threshold=5)
        ...
        if counter.reps == expected_reps:
            self.passed += 1
            print(f"✅ Noise Immunity: {counter.reps} reps (expected {expected_reps})")
        else:
            self.failed += 1
            self.errors.append(f"Noise immunity failed: got {counter.reps}, expected {expected_reps}")
            print(f"❌ Noise Immunity: {counter.reps} reps (expected {expected_reps})")
...
class TestQualityScoring:
    ...
    def test_full_rom_smooth_motion(self):
        ...
        if quality >= 80:
            self.passed += 1
            print(f"✅ Full ROM Smooth Motion: Quality {quality}/100 (>=80 expected)")
        else:
            self.failed += 1
    ...
    def test_limited_rom_lower_quality(self):
        ...
        if quality < 60:
            self.passed += 1
            print(f"✅ Limited ROM: Quality {quality}/100 (<60 expected)")
        else:
            self.failed += 1
    ...
    def test_jerky_motion_lower_quality(self):
        ...
        if quality < 70:
            self.passed += 1
            print(f"✅ Jerky Motion: Quality {quality}/100 (<70 expected)")
        else:
            self.failed += 1
...
def run_all_tests():
    ...
    print("\n" + "="*60)
    print("🧪 BIOMECHANICS MODELS TEST SUITE")
    print("="*60 + "\n")
    ...
    # Repetition Counting Tests
    print("🔄 REPETITION COUNTING TESTS")
    print("-" * 60)
    rep_tests = TestRepetitionCounting()
    ...
    print(f"\nRepetition Tests: {rep_tests.passed} passed, {rep_tests.failed} failed\n")
    ...
    # Summary
    total_passed = angle_tests.passed + rep_tests.passed + quality_tests.passed
    total_failed = angle_tests.failed + rep_tests.failed + quality_tests.failed
